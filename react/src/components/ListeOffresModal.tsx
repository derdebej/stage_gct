...
const ListeOffresModal = ({ setIsModalOpen, onSelectOffre, filterByConsultation = null }) => {
  ...
  useEffect(() => {
    const fetchOffres = async () => {
      try {
        let url = `${baseUrl}/api/offres?search=${encodeURIComponent(search)}`;
        if (filterByConsultation) {
          url += `&id_consultation=${filterByConsultation}`;
        }
