...
const ListeConsultationsModal = ({ setIsModalOpen, onSelectConsultation }) => {
  const [search, setSearch] = useState("");
  const [consultations, setConsultations] = useState<consultationType[]>([]);

  const handleSelect = (consultation: consultationType) => {
      ...
      <div className="bg-white rounded-xl shadow-lg w-full max-w-lg p-6 relative">
        ...
        <div className="flex items-center bg-white px-4 py-2 rounded-full w-full max-w-xl border border-gray-300 my-5">
          ...
          <input
            ...
            placeholder="Rechercher par ID, date, etc."
            ...
            consultations.map((c) => (
              ...
              >
                ...
                <p className="text-xs text-gray-500">
                  Créée le {new Date(c.date_creation).toLocaleDateString()}
                </p>
              </div>
            ))
          ) : (
...
export default ListeConsultationsModal;
